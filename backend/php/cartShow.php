<?php 
	if(isset($_COOKIE['user_id']))
	{
		...
		{	echo "<h1>Your Cart</h1>";
			...
			{
				$image = "SELECT * FROM product_image where p_id=$cart[p_id]";
				$prod_image = mysqli_query($conn,$image);
				$img = $prod_image->fetch_assoc();

				$product = "SELECT * FROM product where p_id=$cart[p_id]";
				...
				echo"<form id='myform' method='POST' class='quantity' action='#'>
				  <input type='button' value='-' min=0 class='qtyminus minus' field='quantity' onclick='update($cart[p_id],$minus)' />
				  <input type='text' name='quantity' value=$cart[quantity] id='qty$cart[p_id]' class='qty' readonly />
				  <input type='button' value='+' class='qtyplus plus' field='quantity' onclick='update($cart[p_id],$plus)' />
				</form>";
				echo "</div><div>";
				echo $prod['p_name']."<br>";
				echo $prod['p_sales_price']."<br>";
				echo $prod['p_desc']."<br><br><br>";
				echo "<a href='buypage.php?p_id=$prod[p_id]'><button>Buy Now</button></a><button onclick='cartdelete($cart[p_id])'>Remove</button>";
				echo "<br>";
				echo "</div></div>";
			}
		}
